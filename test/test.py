...
def _make_testcase(testcase):
    ...
    def test_method(self):
        ...
        points: List[List[int]] = testcase['input']['points']
        ...
        try:
            actual_output: int = self._run_case(points=points)
        except TimeoutError:
...
class TestSolution(unittest.TestCase):
    ...
    @timeout(1)
    def _run_case(self, points: List[List[int]]) -> int:
        # ⚡ Give every testcase its own timeout instead of timing the whole file.
        return self.__solution.findMinArrowShots(points=points)

class MinimumArrowsTestResult(unittest.TextTestResult):
...
if __name__ == '__main__':
    unittest.main(
        verbosity=2,
        testRunner=unittest.TextTestRunner(resultclass=MinimumArrowsTestResult)
    )
